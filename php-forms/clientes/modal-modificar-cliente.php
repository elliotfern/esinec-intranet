...
if (curl_errno($ch)) {
    ...
    $customer_data = json_decode($response);
        ...
        // Verificar si existe información de facturación
        if (isset($customer_data->billing)) {
            $billing = $customer_data->billing;

            // Obtener los datos de dirección y teléfono
            $phone = $billing->phone;
 ...
 // Mostrar los datos en un formulario para modificar             
                        echo '<div class="alert alert-success" id="modificarDatosClienteMessageOk" style="display:none;role="alert">
                        <h4 class="alert-heading"><strong>'.ADD_OK_MESSAGE_SHORT.'</h4></strong>
                        <h6>'.ADD_OK_MESSAGE.'</h6>
                        </div>';
                
                        echo '<div class="alert alert-danger" id="modificarDatosClienteMessageErr" style="display:none;role="alert">
            ...
            echo '<input type="hidden" name="idCliente" id="idCliente" value="'.$idCliente.'">';
...
$(document).ready(function() {
    // Obtener el valor de la provincia del cliente
    var provinciaCliente = '<?php echo $state; ?>';
    ...
    $('#country').on('change', function() {
        ...
        } else if (paisSeleccionado === 'US') {
            var provincias = [
                { value: '', text: 'Selecciona una provincia:' },
                { value: 'NY', text: 'New York' },
                { value: 'CA', text: 'California' },
                { value: 'TX', text: 'Texas' }
                // Agrega más estados de Estados Unidos aquí
            ];
        } else {
            var provincias = [
                { value: '', text: 'Selecciona una provincia:' },
        ...
        if (provincias.length > 0) {
            ...
            // Seleccionar la provincia del cliente
            provinciaSelect.val(provinciaCliente);
    ...
    <?php
            ...
            echo '<input class="form-control" type="text" name="postcode" id="postcode" value="'.$postcode.'">';
            echo '</div>';
            ...
            echo '<input class="form-control" type="text" name="phone" id="phone" value="'.$phone.'">';
            echo '</div>';
